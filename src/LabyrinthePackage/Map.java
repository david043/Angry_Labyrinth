...
class Map
{
private static final boolean WALL = false; //indicates a wall in the maze
private static final boolean PATH = true; //indicates a path in the maze
private boolean[][] boolMap; //Boolean table representing the maze
...
rows = 2*row+1;
cols = 2*col+1;
boolMap = new boolean[rows][cols]; //initializes the feild to proper size
...
boolMap[start_row_index][start_col_index] = PATH;
}
...
if(nxt_row < 0 || boolMap[nxt_row][nxt_col] == PATH){
return 0;
}else{
...
if(nxt_row > rows-1 || boolMap[nxt_row][nxt_col] == PATH){
return 0;
}else{
...
//if the next location is in the array and not already cut, can cut it
if(nxt_col > cols-1 || boolMap[nxt_row][nxt_col] == PATH){
return 0;
}else{
...
int nxt_col = current[1]-2;
//if next location is in the array and not already cut, can cut it
if(nxt_col < 0 || boolMap[nxt_row][nxt_col] == PATH){
return 0;
}else{
...
boolMap[current[0]-1][current[1]] = PATH;
boolMap[current[0]-2][current[1]] = PATH;
loc[0] = nxt_row;
loc[1] = nxt_col;
...
boolMap[current[0]][current[1]+1] = PATH;
boolMap[current[0]][current[1]+2] = PATH;
loc[0] = nxt_row;
loc[1] = nxt_col;
//adds index to stack
tracker.addFirst(loc);
